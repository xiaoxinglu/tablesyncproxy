...
type Config struct {
	...
	TableMappings                 []TableMapping `ignored:"true" yaml:"table_mappings,omitempty"`
	...
	// Single-cluster shorthand (table-sync mode).
	// When cluster_contact_points or cluster_secure_connect_bundle_path is set,
	// origin and target are both pointed at the same cluster automatically.
	// These fields are mutually exclusive with origin_*/target_* fields.

	ClusterContactPoints           string `split_words:"true" yaml:"cluster_contact_points"`
	ClusterPort                    int    `default:"0" split_words:"true" yaml:"cluster_port"`
	ClusterSecureConnectBundlePath string `split_words:"true" yaml:"cluster_secure_connect_bundle_path"`
	ClusterLocalDatacenter         string `split_words:"true" yaml:"cluster_local_datacenter"`
	ClusterUsername                string `split_words:"true" yaml:"cluster_username"`
	ClusterPassword                string `split_words:"true" json:"-" yaml:"cluster_password"`
	ClusterConnectionTimeoutMs     int    `default:"0" split_words:"true" yaml:"cluster_connection_timeout_ms"`
	ClusterTlsServerCaPath         string `split_words:"true" yaml:"cluster_tls_server_ca_path"`
	ClusterTlsClientCertPath       string `split_words:"true" yaml:"cluster_tls_client_cert_path"`
	ClusterTlsClientKeyPath        string `split_words:"true" yaml:"cluster_tls_client_key_path"`

	// Tracing (also known as distributed tracing - request id generation and logging)

	TracingEnabled      bool   `default:"false" split_words:"true" yaml:"tracing_enabled"`
	TracingRateLimit    int    `default:"0" split_words:"true" yaml:"tracing_rate_limit"`
	TracingRequestIdKey string `default:"request-id" split_words:"true" yaml:"tracing_request_id_key"`
...
// applyClusterShorthand copies cluster_* fields into both origin_* and target_*
// when the single-cluster shorthand is used. Called at the start of Validate().
func (c *Config) applyClusterShorthand() error {
	singleCluster := isDefined(c.ClusterContactPoints) || isDefined(c.ClusterSecureConnectBundlePath)
	if !singleCluster {
		return nil
	}

	// Validate() is called twice (LoadConfig + proxy.Start). On the second call
	// origin/target are already populated from the first call — skip the check.
	alreadyApplied := (isDefined(c.ClusterContactPoints) && c.OriginContactPoints == c.ClusterContactPoints) ||
		(isDefined(c.ClusterSecureConnectBundlePath) && c.OriginSecureConnectBundlePath == c.ClusterSecureConnectBundlePath)
	if alreadyApplied {
		return nil
	}

	// Reject mixing single-cluster and dual-cluster fields.
	if isDefined(c.OriginContactPoints) || isDefined(c.OriginSecureConnectBundlePath) ||
		isDefined(c.TargetContactPoints) || isDefined(c.TargetSecureConnectBundlePath) {
		return fmt.Errorf("cluster_contact_points / cluster_secure_connect_bundle_path cannot be combined " +
			"with origin_contact_points / target_contact_points. Use one style or the other.")
	}

	if isDefined(c.ClusterContactPoints) && isDefined(c.ClusterSecureConnectBundlePath) {
		return fmt.Errorf("cluster_contact_points and cluster_secure_connect_bundle_path are mutually exclusive")
	}

	// Mirror into origin and target.
	c.OriginContactPoints = c.ClusterContactPoints
	c.TargetContactPoints = c.ClusterContactPoints
	c.OriginSecureConnectBundlePath = c.ClusterSecureConnectBundlePath
	c.TargetSecureConnectBundlePath = c.ClusterSecureConnectBundlePath
	c.OriginLocalDatacenter = c.ClusterLocalDatacenter
	c.TargetLocalDatacenter = c.ClusterLocalDatacenter
	c.OriginUsername = c.ClusterUsername
	c.TargetUsername = c.ClusterUsername
	c.OriginPassword = c.ClusterPassword
	c.TargetPassword = c.ClusterPassword
	c.OriginTlsServerCaPath = c.ClusterTlsServerCaPath
	c.TargetTlsServerCaPath = c.ClusterTlsServerCaPath
	c.OriginTlsClientCertPath = c.ClusterTlsClientCertPath
	c.TargetTlsClientCertPath = c.ClusterTlsClientCertPath
	c.OriginTlsClientKeyPath = c.ClusterTlsClientKeyPath
	c.TargetTlsClientKeyPath = c.ClusterTlsClientKeyPath
	if c.ClusterPort != 0 {
		c.OriginPort = c.ClusterPort
		c.TargetPort = c.ClusterPort
	}
	if c.ClusterConnectionTimeoutMs != 0 {
		c.OriginConnectionTimeoutMs = c.ClusterConnectionTimeoutMs
		c.TargetConnectionTimeoutMs = c.ClusterConnectionTimeoutMs
	}
...
func (c *Config) Validate() error {
	if err := c.applyClusterShorthand(); err != nil {
		return err
	}
	...
	if err = ValidateTableMappings(c.TableMappings); err != nil {
		return fmt.Errorf("invalid table mappings: %w", err)
	}
