...
type ClientHandler struct {
	...
	tableMappingRegistry *TableMappingRegistry
...
func NewClientHandler(
	...
	return &ClientHandler{
		...
		timeUuidGenerator:                    timeUuidGenerator,
		rateLimiters:                         rateLimiters,
		tableMappingRegistry:                 NewTableMappingRegistry(conf.TableMappings),
...
func (ch *ClientHandler) forwardRequest(request *frame.RawFrame, customResponseChannel chan *customResponse) error {
	...
	// Fire derived INSERT or DELETE for any enabled table mapping, fire-and-forget.
	if err = ch.sendMappedMutationIfNeeded(context, currentKeyspace); err != nil {
		log.Warnf("table-sync: could not send mapped mutation: %v", err)
	}

	return nil
}

// sendMappedMutationIfNeeded inspects the decoded QUERY or BATCH request and, when it
// matches an enabled table mapping for a write statement (INSERT, UPDATE, DELETE), rewrites
// the table reference and dispatches the resulting query as a fire-and-forget internal
// request to the origin connector. Source and target tables are assumed to share the same
// schema.
func (ch *ClientHandler) sendMappedMutationIfNeeded(frameCtx *frameDecodeContext, currentKeyspace string) error {
	f := frameCtx.GetRawFrame()
	switch f.Header.OpCode {
	case primitive.OpCodeQuery:
		return ch.sendMappedQuery(frameCtx, currentKeyspace)
	case primitive.OpCodeBatch:
		return ch.sendMappedBatch(frameCtx, currentKeyspace)
	default:
		return nil
	}
}

// sendMappedQuery handles table-sync for a single QUERY frame (OpCodeQuery).
// This covers plain INSERT/UPDATE/DELETE statements as well as text BATCH
// statements (e.g. BEGIN BATCH ... APPLY BATCH sent by cqlsh), which arrive
// as OpCodeQuery with statementType=batch.
func (ch *ClientHandler) sendMappedQuery(frameCtx *frameDecodeContext, currentKeyspace string) error {
	stmtQueryData, err := frameCtx.GetOrInspectStatement(currentKeyspace, ch.timeUuidGenerator)
	if err != nil {
		return nil // not inspectable; skip silently
	}

	stype := stmtQueryData.queryData.getStatementType()
	ks := stmtQueryData.queryData.getApplicableKeyspace()
	tbl := stmtQueryData.queryData.getTableName()
	log.Debugf("table-sync: opcode=QUERY keyspace=%q table=%q statementType=%v", ks, tbl, stype)

	// Text BATCH (BEGIN BATCH ... APPLY BATCH) sent as a plain OpCodeQuery string.
	// The top-level statementType is "batch"; individual child table names are embedded
	// in the query string. Rewrite all mapped table references in the full query text.
	if stype == statementTypeBatch {
		return ch.sendMappedTextBatch(frameCtx, currentKeyspace, stmtQueryData.queryData.getQuery())
	}

	plan, err := ch.tableMappingRegistry.PlanRewrite(stmtQueryData.queryData)
	if err != nil {
		return fmt.Errorf("table-sync rewrite error for %s.%s: %w", ks, tbl, err)
	}
	if plan == nil {
		return nil
	}

	log.Debugf("table-sync: forwarding %v to %s.%s: %s",
		stype, plan.Mapping.Target.Keyspace, plan.Mapping.Target.Table, plan.TargetQuery)

	decoded, err := frameCtx.GetOrDecodeFrame()
	if err != nil {
		return fmt.Errorf("could not decode frame for table-sync: %w", err)
	}
	origMsg, ok := decoded.Body.Message.(*message.Query)
	if !ok {
		return nil
	}

	targetMsg := &message.Query{
		Query:   plan.TargetQuery,
		Options: origMsg.Options,
	}

	targetFrame := frame.NewFrame(decoded.Header.Version, -1, targetMsg)
	rawTarget, err := ch.getCodec().ConvertToRawFrame(targetFrame)
	if err != nil {
		return fmt.Errorf("could not encode table-sync frame: %w", err)
	}

	return ch.originCassandraConnector.sendRequestToCluster(rawTarget, true)
}

// sendMappedTextBatch handles a text BATCH statement that arrived as OpCodeQuery.
// It rewrites every mapped table reference in the full batch query string and
// forwards the result as a new OpCodeQuery frame.
func (ch *ClientHandler) sendMappedTextBatch(frameCtx *frameDecodeContext, currentKeyspace string, batchQuery string) error {
	rewritten := batchQuery
	matched := false

	for _, mapping := range ch.tableMappingRegistry.bySource {
		replacement := mapping.Target.Keyspace + "." + mapping.Target.Table
		// Try qualified form first, then unqualified.
		if result, n := replaceAllWordCI(rewritten, mapping.Source.Keyspace+"."+mapping.Source.Table, replacement); n > 0 {
			rewritten = result
			matched = true
		} else if result, n := replaceAllWordCI(rewritten, mapping.Source.Table, replacement); n > 0 {
			rewritten = result
			matched = true
		}
	}
	...
	if !matched {
		return nil // no mapping applied
	}

	log.Debugf("table-sync: forwarding text batch: %s", rewritten)

	decoded, err := frameCtx.GetOrDecodeFrame()
	if err != nil {
		return fmt.Errorf("could not decode frame for table-sync text batch: %w", err)
	}
	origMsg, ok := decoded.Body.Message.(*message.Query)
	if !ok {
		return nil
	...
	rawTarget, err := ch.getCodec().ConvertToRawFrame(targetFrame)
	if err != nil {
		return fmt.Errorf("could not encode table-sync text batch frame: %w", err)
	}

	return ch.originCassandraConnector.sendRequestToCluster(rawTarget, true)
}

// sendMappedBatch handles table-sync for a BATCH frame. It iterates each child
// statement, rewrites any that match a mapping, and fires a new BATCH containing
// only the rewritten children — preserving the original batch type and consistency.
func (ch *ClientHandler) sendMappedBatch(frameCtx *frameDecodeContext, currentKeyspace string) error {
	decoded, err := frameCtx.GetOrDecodeFrame()
	if err != nil {
		return fmt.Errorf("could not decode batch frame for table-sync: %w", err)
	}
	origBatch, ok := decoded.Body.Message.(*message.Batch)
	if !ok {
		return nil
	}

	allStmts, err := frameCtx.GetOrInspectAllStatements(currentKeyspace, ch.timeUuidGenerator)
	if err != nil {
		return nil // not inspectable; skip silently
	}

	// Build a list of rewritten child statements for the target batch.
	var targetChildren []*message.BatchChild
	for _, stmtData := range allStmts {
		plan, err := ch.tableMappingRegistry.PlanRewrite(stmtData.queryData)
		if err != nil {
			return fmt.Errorf("table-sync batch rewrite error: %w", err)
		}
		if plan == nil {
			continue // child does not match any mapping
		}
		log.Debugf("table-sync: batch child forwarding %v to %s.%s: %s",
			stmtData.queryData.getStatementType(),
			plan.Mapping.Target.Keyspace, plan.Mapping.Target.Table, plan.TargetQuery)
		targetChildren = append(targetChildren, &message.BatchChild{
			Query:  plan.TargetQuery,
			Values: origBatch.Children[stmtData.statementIndex].Values,
		})
	}

	if len(targetChildren) == 0 {
		return nil // no children matched any mapping
	}

	targetBatch := &message.Batch{
		Type:              origBatch.Type,
		Children:          targetChildren,
		Consistency:       origBatch.Consistency,
		SerialConsistency: origBatch.SerialConsistency,
		DefaultTimestamp:  origBatch.DefaultTimestamp,
	}

	targetFrame := frame.NewFrame(decoded.Header.Version, -1, targetBatch)
	rawTarget, err := ch.getCodec().ConvertToRawFrame(targetFrame)
	if err != nil {
		return fmt.Errorf("could not encode table-sync batch frame: %w", err)
	}
