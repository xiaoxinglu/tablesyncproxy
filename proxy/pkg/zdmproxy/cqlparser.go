...
func buildRequestInfo(
	...
	singleClusterMode bool) (RequestInfo, error) {
	...
	case primitive.OpCodeQuery:
		...
		return getRequestInfoFromQueryInfo(
			...
			forwardSystemQueriesToTarget, virtualizationEnabled, stmtQueryData.queryData, singleClusterMode), nil
	case primitive.OpCodePrepare:
		...
		baseRequestInfo := getRequestInfoFromQueryInfo(
			frameContext.GetRawFrame(), primaryCluster,
			forwardSystemQueriesToTarget, virtualizationEnabled, stmtQueryData.queryData, singleClusterMode)
	...
	default:
		// In single-cluster (table-sync) mode, DDL and other non-write statements
		// must only go to origin. Sending to both would hit the same cluster twice
		// and produce AlreadyExists / duplicate errors for CREATE TABLE etc.
		if singleClusterMode {
			return NewGenericRequestInfo(forwardToOrigin, false, true), nil
		}
...
func getRequestInfoFromQueryInfo(
	...
	virtualizationEnabled bool,
	queryInfo QueryInfo,
	singleClusterMode bool) RequestInfo {
	...
	if queryInfo.getStatementType() == statementTypeSelect {
		...
		// statementTypeOther covers DDL (CREATE TABLE, DROP TABLE, ALTER TABLE, etc.)
		// In single-cluster mode these must only go to origin — sending to both would
		// hit the same node twice and produce AlreadyExists / duplicate errors.
		sendAlsoToAsync = false
		if singleClusterMode {
			forwardDecision = forwardToOrigin
		}
